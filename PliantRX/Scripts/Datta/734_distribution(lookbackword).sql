...
select CASE 
    WHEN rx_dt < from_dt and  rx_dt>=DATEADD(day, -30, from_dt) THEN '-30'
    WHEN rx_dt < DATEADD(day, -30, from_dt) and  rx_dt>=DATEADD(day, -60, from_dt) THEN '-60'
    WHEN rx_dt < DATEADD(day, -60, from_dt) and rx_dt>= DATEADD(day, -90, from_dt) THEN '-90'
    WHEN rx_dt < DATEADD(day, -90, from_dt) and rx_dt>=DATEADD(day, -120, from_dt) THEN '-120'
    WHEN rx_dt < DATEADD(day, -120, from_dt) and  rx_dt>=DATEADD(day, -150, from_dt) THEN '-150'
    WHEN rx_dt < DATEADD(day, -150, from_dt) and  rx_dt>=DATEADD(day, -180, from_dt) THEN '-180'
--    WHEN rx_dt < DATEADD(day, -1080, from_dt) and  rx_dt>=DATEADD(day, -1260, from_dt) THEN '-1260'
--    WHEN rx_dt < DATEADD(day, -1260, from_dt) and  rx_dt>=DATEADD(day, -1440, from_dt) THEN '-1440'
    when  rx_dt < DATEADD(day, -180, from_dt)then 'below -180' 
  END AS treatment_category
  ,count(distinct pat_id)
  from(SELECT a.pat_id, a.from_dt, b.rx_dt
FROM ( 
  SELECT DISTINCT pat_id, min(from_dt ) as from_dt
  FROM pharmetrics.ipf_claims   
  WHERE ('51631' IN (diag_admit, diag1, diag2, diag3, diag4, diag5, diag6, diag7, diag8, diag9, diag10, diag11, diag12)
...
) b ON a.pat_id = b.pat_id  where b.rx_dt<a.from_dt) group by 1
...
select CASE 
    WHEN diag_dt < rx_dt and  diag_dt>=DATEADD(day, -30, rx_dt) THEN '-30'
    WHEN diag_dt < DATEADD(day, -30, rx_dt) and  diag_dt>=DATEADD(day, -60, rx_dt) THEN '-60'
    WHEN diag_dt < DATEADD(day, -60, rx_dt) and diag_dt>= DATEADD(day, -90, rx_dt) THEN '-90'
    WHEN diag_dt < DATEADD(day, -90, rx_dt) and diag_dt>=DATEADD(day, -120, rx_dt) THEN '-120'
    WHEN diag_dt < DATEADD(day, -120, rx_dt) and  diag_dt>=DATEADD(day, -150, rx_dt) THEN '-150'
    WHEN diag_dt < DATEADD(day, -150, rx_dt) and  diag_dt>=DATEADD(day, -180, rx_dt) THEN '-180'
  when  diag_dt < DATEADD(day, -180, rx_dt)then '-180<' 
  END AS treatment_category
  ,count(distinct pat_id)
  from(SELECT distinct a.pat_id, a.diag_dt,a.max_diagdt, b.rx_dt,datediff(day,a.diag_dt,b.rx_dt) as days
FROM (
...
) b ON a.pat_id = b.pat_id where b.rx_dt<a.diag_dt) group by 1;
