...
select * from #temp
where patient_id in 
(select distinct patient_id from #temp where diag_cd in 
(select distinct dx_cd from  pharmetrics.dx_proc_20230523 where 
type_desc in ('Abnormal LFT')))
and patient_id in 
(select distinct patient_id from #temp where diag_cd in 
(select distinct dx_cd from  pharmetrics.dx_proc_20230523 where 
type_desc in ('Hepatic Fibrosis')))
and patient_id in 
(select distinct patient_id from #temp where diag_cd in 
(select distinct dx_cd from  pharmetrics.dx_proc_20230523 where 
...
select count(distinct patient_id) from #temp2
