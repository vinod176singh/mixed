...
(select distinct pat_id,from_dt from 
(select distinct min(from_dt) as from_dt , pat_id from #temp2 where diag_admit in ('J84112','51631') group by 2) 
where extract(year from from_dt)='2018') A 
left join 
...
where extract(year from from_dt) in ('2018') ) C 
...
where extract(year from from_dt) in ('2018') ) F
...
where extract(year from from_dt) in ('2018') ) G
...
where extract(year from from_dt) in ('2018')  ) H
