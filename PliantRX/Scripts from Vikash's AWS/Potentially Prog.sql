...
type_desc in ('Abnormal LFT' ,'Jaundice','Hepatic Fibrosis','Cholangitis')))
or patient_id in (select distinct patient_id from #temp where 
prc_cd in ( select distinct dx_cd from pharmetrics.dx_proc_20230523 where type_desc ='Interventional ERCP' ))

select count(distinct patient_id) from #temp where diag_cd in (select distinct dx_cd from pharmetrics.dx_proc_20230523 where 
type_desc in ('Cholangitis')) 
--
...
type_desc in ('AIH' ,'Cirrhosis','ENCEPHALOPATHY','Varices','Portal Hypertension','ASCITES','Intestinal Cancer','Abdominal Cancer'))))--AIH and Cirhossis
