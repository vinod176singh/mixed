...
where product_id in(select distinct product_id from laad_data.products p where mkted_prod_nm in('OFEV') );

--221 OFEV continuous patient for more than 5 years
...
in(select distinct product_id from laad_data.products p where mkted_prod_nm in('OFEV') )group by 1 )a
inner join 
...
(select distinct patient_id,svc_dt as ofv_dt,claim_id as ofv_claim,product_id as ofv,days_supply_cnt as ofv_dayssup from laad_data.rx_claim 
where patient_id in (select distinct patient_id from #temp8) 
and product_id in ( select distinct product_id from laad_data.products p 
where mkted_prod_nm in('OFEV')) order by patient_id,svc_dt) 

select A.patient_id,B.min_ofv_dt,B.max_ofv_dt,B.ofv_cnt,C.min_pre_dt,C.max_pre_dt,C.pre_cnt,D.sum_ofv_dayssup,E.sum_pre_dayssup from 
(select distinct patient_id from #temp9 )A 
left join
(select patient_id,min(to_date(ofv_dt,'YYYYMMDD')) as min_ofv_dt,max(to_date(ofv_dt,'YYYYMMDD')) as max_ofv_dt,
count(distinct ofv_claim) as ofv_cnt from #temp9 group by 1) B on A.patient_id=B.patient_id
left join
...
left join 
(select patient_id,sum(ofv_dayssup) as sum_ofv_dayssup from #temp9 group by 1) D on A.patient_id=D.patient_id
left join 
